...
def parse_arguments():
    ...
    ap.add_argument(
        "-t", "--title_text", required=False, help="Text displayed in title bar"
    )
    ap.add_argument(
        "-o",
        "--one",
        action="store_true",
        required=False,
        help="Show one station at a time full screen",
    )
    ...
    ap.add_argument(
        "-s", "--savefile", required=False, help="Save screen updates to png file"
    )
    ap.add_argument(
        "-d", "--display", required=False, help="st7789, ili9341, or ili9486"
    )
...
if displaytype == "ili9341":
    ...
    disp = ili9486.ILI9486(
        spi=spi, rst=25, dc=24, origin=ili9486.Origin.LOWER_RIGHT
    ).begin()
    disp.invert()
    spi2 = SpiDev(0, 1)
    spi2.mode = 0b00
...
def get_direction(origin, destination):
    ...
    ix = int(round(bearing / (360.0 / len(dirs))))
...
bluetooth_thread = threading.Thread(
    target=bluetooth_connection_poll_thread, name="btwatch"
)
            ...
            if status == "DCD 0 = 1":
                ...
                draw_current.ellipse(
                    (
                        ...
                        padding,
                        width - title_bar_height - padding * 2,
                        title_bar_height - padding,
                    ),
                    fill=(200, 0, 0, 255),
                )
...
if os.path.exists("/usr/share/fonts/truetype/dejavu/" + fontname):
    ...
    print(
        "Couldn't find font "
        + fontname
        + " in working dir or /usr/share/fonts/truetype/dejavu/"
    )
    exit(1)
